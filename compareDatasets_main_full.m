...
baredges = linspace(0,.1,11); % edge_snr_score_pw.m nEdges
...
%     radial_dists = radial_dist(isite,extension,timeshift);,
    [radial_dists tmp1 tmp2 nEdge SNR] = edge_snr_score_pw_full(isite,extension,timeshift);
    c_signal_woNHarm = [c_signal_woNHarm tmp1];
    nEdges = [nEdges nEdge];
    SNRs = [SNRs SNR];
...
%     c_signal_single = c_signal(:,ind_sort_radial);
    ...
    for i = 1:size(c_signal_single,2)
        ...
        plot(timestamp(tmp2),c_signal_single(:,i),'Color',color(:,color_ind))
    end
