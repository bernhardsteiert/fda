function smoothhist2D(X,lambda,nbins,outliercutoff,plottype,mycolor)
...
%
%   Example 2 (after fPCA):
%         smoothhist2D([x_scores(celltype(ind_harm) == sites_for_harmonics(sites_remain(isite)),:) y_scores(celltype(ind_harm) == sites_for_harmonics(sites_remain(isite)),:)],5,[100 100],.05);
%         hold on
...
%         mymap = hsv;
%         mymap(:,1) = .3;
%         mymap(:,2) = linspace(0,1,256);
%         mymap(:,3) = 1;
%         colormap(hsv2rgb(mymap))
%   
%
%   Reference:
...
%     image(ctrs1,ctrs2,floor(nc.*relF) + 1);
    z = nan(length(ctrs1),length(ctrs2),3);
    z(:,:,1) = mycolor;
    z(:,:,2) = relF;
    z(:,:,3) = 1;
    z = hsv2rgb(z);

%     image(ctrs1,ctrs2,z,'AlphaData',(floor(nc.*relF) + 1),'AlphaDataMapping','scaled');
%     image(ctrs1,ctrs2,z);

%     surf(ctrs1,ctrs2,F,'EdgeAlpha',0,'AlphaData',(floor(nc.*relF) + 1),'AlphaDataMapping','scaled');
    surf(ctrs1,ctrs2,F,z,'EdgeAlpha',0,'AlphaData',(floor(nc.*relF) + 1),'AlphaDataMapping','scaled','FaceAlpha','flat');
    set(gca,'YDir','normal')
    hold on
    % plot the outliers
    if outliercutoff > 0
