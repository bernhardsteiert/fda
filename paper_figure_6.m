...
% posFig(4) = posFig(4)/3;
set(gcf,'Position',posFig)
set(gcf,'PaperPosition', [0 0 posFig(3) posFig(4)]./30);
...
for isite = ligs_to_plot(end:-1:1)
    ...
    plot(x_scores(celltype(ind_harm) == sites_for_harmonics(tmpind(iplot)),:),y_scores(celltype(ind_harm) == sites_for_harmonics(tmpind(iplot)),:),'.','Color',mycolor);
...
set(gca,'YLim',get(gca,'XLim')/(aspRatioFig*aspRatioSubplot)+.05) % PC1 vs PC3
