function [radial_dist c_signal_woNharm range_ind nEdges SNR] = edge_snr_score_pw_full(isite,myextension,timeshift)
    ...
    load('harm_basis_full.mat') % Contains only harm_basis_full from all data-sets
    ...
    time_range = getbasisrange(harm_basis_full);
...
%     c_signal = c_signal - repmat(nanmean(c_signal,2),1,size(c_signal,2));
    ...
    smoothed_additional = smooth_basis(timestamp(range_ind),c_signal(range_ind,:),harm_basis_full);
    
    harm_eval = eval_basis(harm_basis_full,timestamp(range_ind));
