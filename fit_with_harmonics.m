    ...
    for icol = 1:irow
        ...
        for ilig = 1:length(signals)
            if sum(unitypes(ilig)==sites_for_harmonics)
                ...
                legendstyles(ilig) = plot(c_signal_pcastr.harmscr(celltype(ind_harm) == unitypes(ilig),icol),c_signal_pcastr.harmscr(celltype(ind_harm) == unitypes(ilig),irow+1),markers{ilig},'Color',color(ilig,:),'LineWidth',linewidth);
            else
                % Data used for fitting
                legendstyles(ilig) = plot(fitcoef(icol,celltype(ind_fit) == unitypes(ilig)),fitcoef(irow+1,celltype(ind_fit) == unitypes(ilig)),markers{ilig},'Color',color(ilig,:),'LineWidth',linewidth+1);
            end
