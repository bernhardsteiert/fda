...
for iex = 1:length(exclude_site)
    ...
    c_signal = c_signal(:,setdiff(1:size(c_signal,2),tmpi(1)+exclude_signal(iex)-1));
    celltype = celltype(setdiff(1:size(c_signal,2),tmpi(1)+exclude_signal(iex)-1));
end
